from utils import action_benefit, action_cost, action_number, csv_to_list, list_actions
...
def build_matrice(invest_max, actions):
    ...
    """
    matrice = [
        [0 for x in range(invest_max + 1)] for x in range(len(actions) + 1)
    ]  # noqa
    for i in range(1, len(actions) + 1):
        for invest in range(1, invest_max + 1):
            if action_cost(i - 1) <= invest:
                matrice[i][invest] = max(
                    action_benefit(i - 1) + matrice[i - 1][invest - action_cost(i - 1)],
                    matrice[i - 1][invest],
                )
            else:
...
def calculate_optimal_solution(invest_max, actions, matrice, invest):
    ...
    optimal_benefit = [action_benefit(action - 1) for action in optimal_actions]
    total_cost = sum(action_cost(action - 1) for action in optimal_actions)

    return optimal_actions, optimal_benefit, total_cost
...
def optimized_algo(invest_max, actions):
    ...
    (optimal_actions, optimal_benefit, total_cost) = calculate_optimal_solution(
...
optimized_algo(500, data1_sienna)
